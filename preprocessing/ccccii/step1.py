...
def step1_python(case_path):
	...
	#print N
	if N > 30: 
		return case_pixels,np.array([]),np.array([]),np.array([])
        bw = np.copy(bw0)
...
if __name__ == '__main__':
    INPUT_FOLDER = '/new_disk_1/tianchi/data/val_set/'
    patients = [file for file in os.listdir(INPUT_FOLDER) if '.mhd' in file]
    patients.sort()
    case_pixels, m1, m2, spacing = step1_python(os.path.join(INPUT_FOLDER,'LKDS-00276.mhd'))
    
    np.save('case_pixels',case_pixels)
    np.save('m1',m1)
    np.save('m2',m2)
    #plt.imshow(m1[60])
    #plt.figure()
