...
config = {'stage1_data_path':'/work/DataBowl3/stage1/stage1/',
          'luna_raw':'data/train_set/',
          'luna_segment':'/new_disk_1/tianchi/data/train_lungSeg_npy/',
          'val_segment':'/new_disk_1/tianchi/data/val_lungSeg_npy/',
          'luna_data':'/new_disk_1/tianchi/data/train_set/',
          'val_data':'/new_disk_1/tianchi/data/val_set/',
          'preprocess_result_path':'/home/datasets/imagenet/tianchi/lfz_data_train/',
          'val_result_path':'/home/datasets/imagenet/tianchi/lfz_data_val/',
          'luna_abbr':'./detector/labels/shorter.csv',
          'luna_label':'/new_disk_1/tianchi/data/csv/train/annotations.csv',
          'val_label':'/new_disk_1/tianchi/data/csv/val/annotations.csv',
...
#    savepath = '../data/'
#    luna_data = '../../../../data/train_set/'
#    luna_segment = '../../../../data/train_set/'
#    name = 'LKDS-00995'
#   annos = np.array(pandas.read_csv('../../../../data/csv/train/annotations.csv'))
    print name,os.path.join(luna_segment,name+'.mhd.npy') 
    if not os.path.exists(os.path.join(luna_segment,name+'.mhd.npy')) : 
    	return 1
    Mask = np.load(os.path.join(luna_segment,name+'.mhd.npy'))
#    print Mask,origin,spacing,isflip
#    np.save('Mask',Mask)
#    if isflip:
#        Mask = Mask[:,::-1,::-1]
    newshape = Mask.shape
	...
	sliceim = sliceim*np.load(os.path.join(luna_segment,name+'.mhd.npy'))
...
def preprocess_luna():
    ...
    if not os.path.exists(finished_flag):
        ...
        #filelist = [f.split('.')[0] for f in os.listdir(luna_data)]
        filelist = imgs
        print(filelist)
